...
// make x: 500 the middle
// what is index of 479 if 500 is middle?
// 450 500 550
...
int does_sand_block_source(char map[MAP_HEIGHT][MAP_WIDTH], int x, int y);
int is_sand_on_floor(position current_position);
int is_sand_blocking_source(position current_position);
...
int main(void)
{
    ...
    while (!does_sand_block_source(map, SAND_SOURCE_X, SAND_SOURCE_Y))
    {
        grain_count++;
    }
...
void initialize_map(char map[MAP_HEIGHT][MAP_WIDTH], FILE *fp)
{
    ...
    // draw a line at the bottom of the graph that represents the floor
...
int does_sand_block_source(char map[MAP_HEIGHT][MAP_WIDTH], int x, int y)
{
    ...
    while (can_move)
    {
        if (is_sand_on_floor(current_position))
        {
            can_move = 0;
            break;
        }
    ...
    if (is_sand_blocking_source(current_position))
    {
        return 1;
    }
...
int is_sand_blocking_source(position current_position)
{
    return current_position.x == SAND_SOURCE_X && current_position.y == SAND_SOURCE_Y;
}

int is_sand_on_floor(position current_position)
{
    return current_position.y == FLOOR_Y - 1;
}